...
from common.row_utils import clean_csv_row, calculate_row_hash, row_to_json_string
...
def ensure_raw_eu_fsf_table(postgres: PostgresClient) -> None:
    ...
    postgres.execute(
        """
        CREATE TABLE IF NOT EXISTS raw.eu_fsf_full (
...
def drop_raw_eu_fsf_indexes(postgres: PostgresClient) -> None:
    postgres.execute(
        """
        DROP INDEX IF EXISTS raw.idx_eu_fsf_full_row_hash;
        DROP INDEX IF EXISTS raw.idx_eu_fsf_full_load_date;
        DROP INDEX IF EXISTS raw.idx_eu_fsf_full_source_object;
        """
    )
...
def create_raw_eu_fsf_indexes(postgres: PostgresClient) -> None:
    postgres.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_eu_fsf_full_row_hash
            ON raw.eu_fsf_full (row_hash);

        CREATE INDEX IF NOT EXISTS idx_eu_fsf_full_load_date
            ON raw.eu_fsf_full (source_load_date);

        CREATE INDEX IF NOT EXISTS idx_eu_fsf_full_source_object
            ON raw.eu_fsf_full (source_object_key);
        """
    )
...
# -------------------------------------------------------------------
# Database helpers (mit commit=False für Transaktionssicherheit)
# -------------------------------------------------------------------
...
def delete_existing_rows_for_object(
    ...
    postgres.execute(
        """
        DELETE FROM raw.eu_fsf_full
        WHERE source_object_key = %s
        """,
        (source_object_key,),
        commit=False,  # Steuerung über transaction()
    )
...
def main() -> None:
    ...
    print("Target table: raw.eu_fsf_full")
    print("Insert mode: PostgreSQL COPY (ELT style)")
    ...
    try:
        ...
        for file_entry in success_files:
            ...
            print("Writing to table: raw.eu_fsf_full")

            file_row_count = 0
            file_inserted_count = 0

            # Transaktionsblock: Erst wenn dieser ohne Exception durchläuft, wird am Ende committet
